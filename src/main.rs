...
struct Sphere {
    pub radius: f64,
    pub position: Vec3, // ~\~ begin <<lit/index.md|sphere-members>>[init]
    ...
    loop {
        ...
        match object.reflection {
            Reflection::Diffuse => {
                ...
                let d =
                    (u * phi.cos() * r + v * phi.sin() * r + n_refl * (1. - r2).sqrt()).normalize();
                // ~\~ end
...
pub struct Image {
    ...
    pub(crate) data: Vec<RGBColour>,
}

impl Image {
    fn new(width: usize, height: usize, colour: Option<RGBColour>) -> Image {
        Image {
            width,
            height,
            data: vec![colour.unwrap_or(BLACK); width * height],
        }
    }

    fn for_each<F>(&mut self, f: F)
    where
        F: Send + Sync + Fn(usize, usize, &mut RGBColour),
    {
        let w = self.width;
...
mod render_view;
use render_view::render_window;
use std::sync::mpsc::channel;

use crate::render_view::LitUi;
...
fn main() -> std::io::Result<()> {
    ...
    if args.window {
        let bg = Image::new(w, h, Some(GREY));
        let (tx, rx) = channel();
        let lit_ui = LitUi { rx, img: bg.into() };
    }
    ...
    let mut img = Image::new(w, h, None);
    println!("Rendering ({} spp)", samps * 4);

    let (tx, rx) = channel::<(usize, usize, RGBColour)>();
        ...
        img.for_each(|x, y, c| {
            ...
            for sy in 0..2 {
                for sx in 0..2 {
                    ...
                    *c = *c + r.clamp() * 0.25;
                    let guardian = tx.lock().unwrap();
                    guardian.send((x, y, *c)).expect("Send failed"); // TODO Maybe a shallow Copy will do.
                }
            }
        });
        println!("\nWriting image.");
        img.print_ppm("image_rust.ppm").unwrap();
    });

    if args.window {
        let bg = Image::new(w, h, Some(GREY));
        let lit_ui = LitUi { rx, img: bg.into() };
        render_window(lit_ui);
    }

    handle.join().expect("joining threads failed. All tied up!");
    Ok(())
}
// ~\~ end
